...
# This function is not strictly needed anymore if we extract index separately,
# but kept here for potential future use or clarity if the format might vary.
...
def extract_queries_from_file(input_filename):
    ...
    # Pattern to find preferindex (to ignore lines that might contain it)
    ...
    with open(input_filename, 'r', encoding='utf-8') as infile:
        ...
        while i < len(lines):
            ...
            # Skip standalone preferindex lines entirely now
            if preferindex_pattern.match(line) and not quiri_pattern.match(line):
                i += 1
                continue
            ...
            # --- Handle quiri/query lines (index part is ignored) ---
...
# --- Write the final CSV ---
if all_rows_data:
    # Define CSV header order - MAKE SURE THIS MATCHES THE FEATURE EXTRACTION KEYS
    csv_headers = ['quiri'] + [f"{feat.replace(' ', '_')}_count" for feat in sql_features_list] + ['target'] # *** CORRECTED LINE ***
